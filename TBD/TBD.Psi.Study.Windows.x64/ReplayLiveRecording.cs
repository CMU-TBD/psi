namespace TBD.Psi.Study
{
    ...
    public class ReplayLiveRecording
    {
        public static void Run()
        {
            ...
            {
                // input & output store
                var inputStorePath = Path.Combine(Constants.OperatingDirectory, Constants.PartitionIdentifier, Constants.ReplayRecordingPath);
                var inputStore = PsiStore.Open(p,Constants.TestRecordingPath.Split(Path.DirectorySeparatorChar).Last().Split('.')[0], inputStorePath);
                var outputStorePath = Path.Combine(Constants.OperatingDirectory, Constants.PartitionIdentifier, @"live-recording-replay");
                var outputStore = PsiStore.Create(p, "live-recording-replay", Path.Combine(Constants.OperatingDirectory, outputStorePath));
                ...
                // add kinect 2 bodies into the body mergers
                foreach (var k2BodyStreamName in inputStore.AvailableStreams.Where(s => s.Name.StartsWith("k2") && s.Name.EndsWith("bodies")).Select(s => s.Name))
                {
