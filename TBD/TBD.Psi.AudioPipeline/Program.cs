    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var p = Pipeline.Create(true))
            {
                ...
                // capture the audio from mic in wav form
                var audioCapture = new AudioCapture(p, WaveFormat.Create16kHz1Channel16BitPcm());

                // set up the audio vad
                var audioVAD = new SystemVoiceActivityDetector(p);

                // send the mic audio to the VAD for voice deteaction
                audioCapture.Out.PipeTo(audioVAD.In);

                // join the voice detection stream with audio from mic
                var fused = audioVAD.Out.Join(audioCapture.Out);
                ...
                audioCapture.Out.PipeTo(audioFileWriter);
                ...
                audioCapture.Write("Audio", store);
                audioVAD.Write("AudioVAD", store);
                fused.Out.Write("Joined", store);
